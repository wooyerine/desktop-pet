...
/* ---------------- 햄스터 (정면, 동글납작 + 작은 귀) ----------------
 * 목 없이 머리에서 몸통으로 바로 이어지는 통통한 실루엣 */
const HAMSTER_SIL = [
  [[4, 6], [13, 15]],           // 작고 동그란 귀
  [[3, 16]],                    // 머리 위
  [[2, 17]],
...
const HAMSTER_PAINT = [
  ...
  [7, 5, 14, 'w'],
  [8, 7, 12, 'w'],
  // 분홍 볼 (흰 얼굴 안쪽에 — 테두리에 걸치면 잘려 보인다)
  [6, 4, 5, 'p'], [6, 14, 15, 'p'],
  // 턱은 좁게, 9행은 금색으로 남겨 얼굴과 배를 갈라 준다
  // (안 그러면 머리부터 배까지 흰 덩어리 하나로 보인다)
  [10, 4, 15, 'w'],
  [11, 4, 15, 'w'],
  [12, 4, 15, 'w'],
  [13, 5, 14, 'w'],
...
const OTTER_PAINT = [
  // 정수리만 진한 갈색 — 귀는 밝은 기본색으로 남겨야 머리와 구분된다
  [1, 4, 15, 'J'],
  [2, 3, 16, 'J'],
  [3, 2, 5, 'J'], [3, 14, 17, 'J'],
  [4, 2, 3, 'J'], [4, 16, 17, 'J'],
  // 크림 주둥이 — 얼굴 전체가 아니라 코 둘레만 동그랗게 (해달의 특징)
  [6, 7, 12, 'i'],
  [7, 6, 13, 'i'],
  [8, 7, 12, 'i'],
  // 크림 가슴
...
const PET_DEFS = {
  ...
  hamster: {
    ...
    paw: ['.kkk.', 'kpppk', 'kwppk', '.kkk.'],
    eyes: { lx: 6, rx: 12, y: 5 },
    face(dy) {
      // 아주 작은 코
      rect(PET_X + 9, PET_Y + dy + 7, 2, 1, 'k');
    },
  },
  otter: {
    ...
    face(dy) {
      // 넓적한 코 + 입 갈라짐
      rect(PET_X + 9, PET_Y + dy + 6, 2, 1, 'k');
      px(PET_X + 9, PET_Y + dy + 7, 'J');
      px(PET_X + 10, PET_Y + dy + 7, 'J');
    },
  },
};
